...
TrainingDataPath = "/data/yyl/teco-video/"
...
# Directories
Flags.DEFINE_string('input_dir_LR', "/data/yyl/teco-video/",
...
Flags.DEFINE_string('output_dir', train_dir, 'The output directory of the checkpoint')
...
Flags.DEFINE_string('summary_dir', os.path.join(train_dir, "log/"), 'The dirctory to output the summary')

# Models
Flags.DEFINE_string('checkpoint', FRVSRModel, 'If provided, the weight will be restored from the provided checkpoint')
Flags.DEFINE_integer('num_resblock', 16, 'How many residual blocks are there in the generator')
# Models for training
Flags.DEFINE_boolean('pre_trained_model', True, 'If True, the weight of generator will be loaded as an initial point'
                                                'If False, continue the training')
Flags.DEFINE_string('vgg_ckpt', VGGModelPath, 'path to checkpoint file for the vgg19')

# Machine resources
Flags.DEFINE_string('cudaID', '1', 'CUDA devices')
Flags.DEFINE_integer('queue_thread', 12, 'The threads of the queue (More threads can speedup the training process.')
Flags.DEFINE_integer('name_video_queue_capacity', 1024, 'The capacity of the filename queue (suggest large to ensure'
                                                        'enough random shuffle.')
Flags.DEFINE_integer('video_queue_capacity', 1024, 'The capacity of the video queue (suggest large to ensure'
                                                   'enough random shuffle')
...
Flags.DEFINE_string('input_video_dir', TrainingDataPath, 'The directory of the video input data, for training')
...
Flags.DEFINE_integer('end_dir', 2220, 'The ending index of the video directory')
Flags.DEFINE_integer('end_dir_val', 2274, 'The ending index for validation of the video directory')
Flags.DEFINE_integer('max_frm', 119, 'The ending index of the video directory')
# The loss parameters
Flags.DEFINE_float('vgg_scaling', 0.2, 'The scaling factor for the VGG perceptual loss, disable with negative value')
Flags.DEFINE_float('warp_scaling', 1.0, 'The scaling factor for the warp')
Flags.DEFINE_boolean('pingpang', True, 'use bi-directional recurrent or not')
Flags.DEFINE_float('pp_scaling', 0.5, 'factor of pingpang term, only works when pingpang is True')
# Training parameters
Flags.DEFINE_float('EPS', 1e-12, 'The eps added to prevent nan')
Flags.DEFINE_float('learning_rate', 0.00005, 'The learning rate for the network')
